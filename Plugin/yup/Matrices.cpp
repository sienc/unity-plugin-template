...
void Matrix4::getEuler(float& yaw, float& pitch, float& roll) const
{
	if (get(0, 0) == 1.0f)
	{
		yaw = atan2f(get(0, 2), get(2, 3));
		pitch = 0;
		roll = 0;

	}
	else if (get(0, 0) == -1.0f)
	{
		yaw = atan2f(get(0, 2), get(2, 3));
		pitch = 0;
		roll = 0;
	...
	{
		yaw = atan2(-get(2, 0), get(0, 0));
		roll = asin(get(1, 0));
		pitch = atan2(-get(1, 2), get(1, 1));
	}
}

float Matrix4::getHeading() const
{
	float yaw;
